...
```
SurveyProtocol/
├── ResearchVR2.csv              # 生データ（全DB統合エクスポート, 14,385件）
...
│
├── step1_dedup.csv              # Phase 1 出力: 重複削除済み（12,442件）
├── step2_rank_included.csv      # Phase 2 出力: 高ランクVenue通過（2,858件）
├── step2_rank_excluded.csv      # Phase 2 出力: 低ランクVenue除外（9,584件）
├── step3_kw_included.csv        # Phase 3 出力: ★最終候補（1,784件）
├── step3_kw_excluded.csv        # Phase 3 出力: キーワード除外（1,074件）
│
├── pipeline_log.txt             # パイプライン実行ログ（詳細）
├── rule,txt                     # 研究プロトコル・方針文書
└── README.md                    # このファイル
```
...
| SJR Q1 | 1,912件 |
| **通過合計** | **2,858件** |
| CORE B/C（低ランク） | 2,728件 |
| SJR Q2/Q3/Q4 | 1,730件 |
| ランク未判定（Unmatched） | 5,126件 |
| **除外合計** | **9,584件** |
...
│   └─ 重複削除後  : 12,442 件
│
├─ Phase 2 学会ランクスクリーニング ──── -9,584件
│   └─ 高ランク通過: 2,858 件
│
└─ Phase 3 キーワード除外 ───────────── -1,074件
    └─ 最終候補    : 1,784 件  ← step3_kw_included.csv
```
...
`simulate_screening.py` は **step3_kw_included.csv（1,784件）** に対して、  
...
*最終更新: 2026-05-25 | プロトコル: PRISMA 2020 準拠*