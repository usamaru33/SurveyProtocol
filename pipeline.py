...
# メタデータ品質による除外は正当化できないので、判定不能は保留し人手へ送る。
#
# 実測値(2026-08-23 再測定、統合コーパス18,342件。旧コメントの「567件中566件」は
# 第1波のみの古い値で、原稿にもそのまま転記されていたので是正した):
#   - 要旨欠落 3,707件。うちタイトルのみで3概念群が成立するものは **0件**
#     → タイトル判定にすると要旨欠落レコードは全滅する。
#   - gold set 17件のうちコーパスに在る13件は**全件が要旨を持つ**ので、
#     本フェイルセーフが直接救う known item は 0件。
#     ただしタイトルのみ判定だと13件中4件しか残らない(=9件を失う)。
#     本層で落ちる2件(#24 okada2025, #25 pouke2020)は要旨を持っており、
#     メタデータではなく語彙(概念群不成立)で落ちている。
# 再測定は scripts/audit_manuscript_figures.py が常時検証する。
# ---------------------------------------------------------------------------
