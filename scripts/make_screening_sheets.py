...
# 判定対象の一定割合を決定論的に抽出して校正セットとする。
# (副次的な効果として、本作業前の判断基準のすり合わせにもなる)
#
# 【2026-08-20 Rev.22: 15 → 20 に変更】
# Cochrane Rapid Reviews Methods Group の更新ガイダンス(`rule.md` §5 [R4]
# Nussbaumer-Streit et al. 2023)が二重判定の割合として **20%** を例示しており、
# 15% はこれを下回っていた。先行研究の水準に合わせる。
#
# ★ この閾値は **単調** である。md5%100 < 15 の集合は < 20 の集合に完全に含まれる。
#   したがって割合を上げても **既に校正セットだった文献が外れることはない**。
#   配布済みシートの判定を捨てずに、差分(59件)を補足シートとして追加できる。
#   割合を下げる場合はこの性質が働かないので、既存の判定が無駄になる。
CALIBRATION_PCT = 20
