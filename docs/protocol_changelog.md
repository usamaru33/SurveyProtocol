...
## 2026-08-01 — Rev.9: Phase 4 の評価者体制を3名に確定(一致度統計は要決定)

> **Rev番号は暫定。** 検索 scope の変更(TA → TAK、ACM/IEEE 第2波の件数急減を受けた検討中の案)を
> 先に確定させる場合は、そちらを Rev.9 とし本項を Rev.10 に繰り下げること。

### 変更内容

**Phase 4(全文適格性評価)の評価者を3名に確定した。**
...
1. **一致度統計の選択:** `rule.md` Rev.2 は Phase 3b/4 とも **Cohen's κ** を前提としているが、
   Cohen's κ は**2評価者専用**であり3名にはそのまま適用できない。以下のいずれかを選ぶ必要がある。
   - **(a) Fleiss' κ** — 3名全員が全件を独立評価する場合。多数決で採否を決め、不一致は協議。
     PRISMA での報告が最も素直だが、工数は全件×3。
   - **(b) ペアワイズ Cohen's κ の平均** — 各文献を2名が担当する分担方式。工数は全件×2に抑えられるが、
     「どのペアがどの文献を見たか」の割当記録が必須になる。
2. **割当方式:** 全件×3名か、2名重複×3ペアの分担かが未定（上記と連動）。
3. **Phase 3b の扱い:** 本改訂は Phase 4 のみの決定。Phase 3b(Title/Abstract 二重スクリーニング)を
   同じ3名で行うのか、従来どおり2名で行うのかは**未確定**。3名で行う場合は Phase 3b の
   一致度統計も同様に変更が必要。
4. **`rule.md` 本文への反映:** Phase 4 の評価者数・独立性・一致度統計の記述を更新すること。
   反映は Rev.8 の未反映分(3DB構成・scope=TA・Threats)とまとめて行う予定。
