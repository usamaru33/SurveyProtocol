...
【何を】
著者が stage 1(全1,052件)の判定を終えたあと、**著者が Exclude または Unsure にした
レコードだけ**を第2評価者(Kataoka / WATANABE)に配るシートを生成する。
Include は1人の判断で通すので配らない。

【なぜ Include を配らないのか — liberal accelerated】
...
Unsure も第2評価者へ回す。著者が判断できなかったものを1人の判断で確定させないため、
かつ協議(consensus_worklist.csv)の材料にするためである。

単独スクリーニングの見落とし率(1人 13% / 2人 3%)の出典は `docs/protocol/rule.md` §5 [R1]。
