...
> 1. **プロトコルは Rev.11 まで進んでいるが、`step*.csv` は Rev.6 以前の状態**。公式再実行は
...
> 3. プロトコルの決定経緯・最新方針は `docs/protocol_changelog.md`（〜Rev.11）と
...
└── docs/                        # プロトコル文書（2026-07-21 集約）
    ...
    ├── protocol_changelog.md    # ★プロトコル変更履歴（〜Rev.11）— 方針の一次情報
